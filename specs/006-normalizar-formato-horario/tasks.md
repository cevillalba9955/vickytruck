...
- [ ] T034 [P] Ejecutar `quickstart.md` sección "Verificación técnica del contrato (Oracle)" contra un entorno Oracle/APEX de prueba — validación manual obligatoria de `leer_estado_puntos` (marcada previamente como no probada contra el backend real, ver research.md Decisión 4) — **PENDIENTE**: requiere acceso a un entorno Oracle/APEX real, no ejecutable desde este entorno de desarrollo
