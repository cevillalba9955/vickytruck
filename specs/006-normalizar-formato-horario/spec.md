...
Un chofer abre la app y consulta la hora de arribo y descarga registrada en cada punto de entrega, así como la ventana horaria de entrega del cliente. Necesita que esa hora coincida con la hora real de su reloj/celular (hora de Argentina), sin ambigüedad de AM/PM ni desfasajes por zona horaria, y con un formato consistente en toda la pantalla.
...
**Independent Test**: Puede probarse abriendo la app del chofer, comparando cada horario mostrado (arribo, descarga, ventana de entrega) contra la hora real de un reloj de referencia en Argentina, y verificando que todos usan el mismo formato de 24 horas.
...
3. **Given** la ventana horaria de entrega de un cliente, **When** el chofer la visualiza, **Then** ambos extremos del rango se muestran en el mismo formato de 24 horas y con ceros a la izquierda que el resto de los horarios de la app.
...
- ¿Qué sucede con una ventana horaria de entrega (rango) que llega desde el origen de datos en un formato distinto al esperado (ej. con un solo dígito de hora, o con separador distinto)? Debe normalizarse al formato estándar antes de mostrarse, o señalarse de forma visible si no puede interpretarse, en lugar de mostrarse tal cual.
...
- **FR-004**: La ventana horaria de entrega de un cliente (rango "desde–hasta") DEBE mostrarse usando el mismo estilo de 24 horas con ceros a la izquierda que el resto de los horarios de la app, para no mezclar convenciones dentro de la misma pantalla.
...
- **FR-006**: Al aplicar el nuevo formato de visualización, el sistema DEBE preservar el instante real originalmente registrado de cada horario histórico ya existente; la normalización solo cambia cómo se muestra la hora, no el momento real que representa.
- **FR-007**: El sistema DEBE mostrar de forma consistente los horarios que se intercambian entre la app del chofer, el panel de Central y el backend (incluyendo los eventos de ubicación transmitidos en tiempo real), de modo que todas las partes interpreten el mismo instante de la misma manera al presentarlo al usuario final.
...
- **SC-001**: El 100% de los horarios visibles para choferes y operadores de Central se presentan en formato de 24 horas con segundos (HH24:MM:SS), sin ninguna pantalla remanente en formato de 12 horas o sin segundos.
...
- **SC-004**: Los horarios de viajes históricos, registrados antes del cambio, se visualizan correctamente en el nuevo formato sin alterar el instante real que representan, verificado sobre una muestra de viajes cerrados previos.
...
- La normalización de formato incluye tanto lo que se muestra en pantalla (app chofer y panel de Central) como los horarios que viajan entre los sistemas del backend (API, eventos MQTT de ubicación), de modo que el problema de fondo —falta de una zona horaria consistente entre el origen de datos y lo que ve el usuario— quede resuelto y no solo maquillado en la última capa visual.
...
- La ventana horaria de entrega (rango) no requiere precisión de segundos porque el dato de origen no la tiene; se normaliza su estilo (24 horas, ceros a la izquierda) pero mantiene granularidad de minutos.
