...
- [ ] T001 [P] Agregar dependencia `mqtt` (mqtt.js) a `backend/package.json`
- [ ] T002 [P] Agregar dependencia `mqtt` (mqtt.js) a `frontend/package.json`
- [ ] T003 [P] Agregar dependencia `mqtt` (mqtt.js) a `central/package.json`
- [ ] T004 [P] Crear estructura de directorios `backend/src/mqtt/` (para `client.js`,
      `subscriber.js`, `emqxProvisioning.js`) y `backend/scripts/` (ya existe, se
      reutiliza) per plan.md
- [ ] T005 [P] Documentar en `backend/.env.example` las variables nuevas: host/puerto
...
- [ ] T006 Implementar `backend/src/mqtt/client.js`: conexión saliente `mqtts://` hacia
      ...
      de T001, T005)
- [ ] T007 [P] Implementar `backend/src/mqtt/emqxProvisioning.js`:
...
- [ ] T008 Conectar el arranque (y cierre ordenado) de `mqtt/client.js` a
      `backend/src/server.js` (depende de T006)
- [ ] T009 [P] Implementar `frontend/src/services/mqttClient.js`: `connect(config)` (wss,
      credenciales por token) y `publish(topic, payload, opciones)` sobre `mqtt.js`
      (depende de T002)
- [ ] T010 [P] Implementar `central/src/services/mqttClient.js`: `connect(config)` (wss,
      credencial de servicio) y `subscribe(topicFilter, onMessage)` sobre `mqtt.js`
      (depende de T003)
- [ ] T011 [P] Unit test de `emqxProvisioning.js` contra un fake de la API de
...
- [ ] T012 [P] [US1] Contract test: `GET /api/recorridos/:token` incluye el bloque
...
- [ ] T013 [P] [US1] Integration test: un mensaje recibido en
...
- [ ] T014 [US1] Integrar `emqxProvisioning.provisionarCredencial(token)` al confirmar
      `asignar`/`reasignar` en `backend/src/db/centralRepository.js` (depende de T007)
- [ ] T015 [US1] Extender `GET /api/recorridos/:token` para incluir `recorrido.mqtt`
      ...
      T014)
- [ ] T016 [US1] Retirar el handler `POST /:token/ubicacion` de
      `backend/src/routes/recorrido.js` (FR-001) (depende de T015; comparte archivo)
- [ ] T017 [US1] Implementar `backend/src/mqtt/subscriber.js`: suscripción a
...
- [ ] T018 [US1] Conectar `mqtt/subscriber.js` al arranque del servidor en
      `backend/src/server.js` (depende de T017; comparte archivo con T008)
- [ ] T019 [US1] Adaptar `frontend/src/services/ubicacionPeriodica.js` para publicar vía
      `mqttClient.publish(ubicacionTopic, {lat, lon, en}, {qos: 0, retain: true})` en vez
      de `POST /:token/ubicacion` (depende de T009)
- [ ] T020 [US1] Inicializar `mqttClient` en `frontend/src/main.jsx` con la configuración
      `recorrido.mqtt` recibida de `GET /:token`, antes de arrancar
      `iniciarReportePeriodico` (depende de T009, T019)
- [ ] T021 [US1] Implementar `GET /api/central/mqtt-config` en
...
- [ ] T022 [US1] Inicializar `central/src/services/mqttClient.js` al cargar `central/`
      ...
      panel (depende de T010, T021)
- [ ] T023 [P] [US1] Service test: `frontend/src/services/mqttClient.js` conecta, publica
      con `retain` y reintenta reconexión, con `mqtt.js` mockeado, en
      `frontend/tests/services/mqttClient.test.js`
- [ ] T024 [P] [US1] Service test: `central/src/services/mqttClient.js` se suscribe y
...
- [ ] T025 [P] [US2] Integration test: un mensaje en
      ...
      (FR-013) en `backend/tests/integration/mqtt-eventos.test.js`
- [ ] T026 [P] [US2] Service test: la cola de reintento offline del frontend encola una
      acción de arribo/descarga sin conectividad y la publica automáticamente al
      reconectar, sin duplicar (FR-010) en `frontend/tests/services/offlineQueue.test.js`
      (extiende el test ya existente de 001)

### Implementation for User Story 2

- [ ] T027 [US2] Extender `backend/src/mqtt/subscriber.js`: suscripción a
...
- [ ] T028 [US2] Retirar los handlers `POST /:token/puntos/:puntoId/arribo` y
      `.../descarga` de `backend/src/routes/recorrido.js` (FR-002) (depende de T016;
      comparte archivo)
- [ ] T029 [US2] Adaptar `frontend/src/components/DeliveryPointCard.jsx` y
      `frontend/src/services/api.js` para publicar la acción vía
      `mqttClient.publish(eventosTopic, {tipo, puntoId, lat, lon}, {qos: 1})` en vez de
      `POST` (depende de T009, T020)
- [ ] T030 [US2] Adaptar `frontend/src/services/offlineQueue.js` para reintentar
      publicaciones MQTT (en vez de `POST` HTTP) al recuperar conectividad, sin duplicar
      ni perder acciones (FR-010) (depende de T029)
...
- [ ] T031 [P] [US3] Extender el unit test de `emqxProvisioning.js`:
      `revocarCredencial` no lanza si la credencial ya no existe (idempotente) en
      `backend/tests/unit/emqxProvisioning.test.js`
- [ ] T032 [P] [US3] Integration test: al reasignar un recorrido, la credencial MQTT del
      token anterior se revoca y se provisiona una nueva para el nuevo token en
      `backend/tests/integration/reasignar-credencial-mqtt.test.js`

### Implementation for User Story 3

- [ ] T033 [P] [US3] Script idempotente que crea la regla de ACL global (`PUBLISH` en
      ...
      `backend/scripts/smoke-oracle-connection.js`)
- [ ] T034 [US3] Integrar `emqxProvisioning.revocarCredencial(tokenAnterior)` en el flujo
      de reasignación (`centralRepository.reasignar`) en
      `backend/src/db/centralRepository.js`, antes de provisionar la credencial del nuevo
      token (depende de T014, T007)
- [ ] T035 [US3] Detectar en `backend/src/mqtt/subscriber.js` cuando un evento de
...
- [ ] T036 [P] Actualizar/retirar los contract tests de 001 para los endpoints
      `POST .../ubicacion`, `.../arribo` y `.../descarga` (ya no existen) en
      `backend/tests/contract/post-*.test.js`
- [ ] T037 [P] Documentar en `backend/README.md` el flujo operativo de alta de EMQX Cloud
      (tier Serverless, ejecutar `emqx-setup.js` una vez, variables de entorno nuevas de
      `.env.example`)
- [ ] T038 Ejecutar manualmente los Escenarios de validación 1 a 4 de quickstart.md de
      punta a punta
