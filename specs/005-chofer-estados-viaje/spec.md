...
- **Enmienda de constitución ya aplicada**: el Principio II prohibía que el chofer reordenara los puntos. Se enmendó (versión 3.0.0 → 4.0.0, redefinición incompatible) para permitir explícitamente que el chofer mueva puntos pendientes de su propio recorrido activo al primer puesto mediante IR PRIMERO, sincronizado como orden autoritativo hacia Central/Oracle (ver `.specify/memory/constitution.md`, Principio II). FR-016 ya no está en conflicto con la constitución vigente.
